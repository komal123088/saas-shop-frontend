...
export default function InventoryPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await api.patch(ENDPOINTS.PAYMENT(selectedEntry._id), {
        amount,
      });
      setEntries(
        entries.map((e) => (e._id === selectedEntry._id ? res.data : e)),
      );
      setPaymentAmount("");
  ...
  const exportCSV = () => {
    ...
    entries.forEach((e) => {
      csv += `${safeDate(e.purchaseDate)},"${e.supplierName}",${e.supplierPhone},"${e.itemDescription}",${e.quantity},${e.unit},RS${e.purchasePrice},RS${e.totalAmount},RS${e.amountPaid},RS${e.remainingAmount},"${e.notes}"\n`;
    });
                    ...
                    <div className="row g-2 text-center">
                      <div className="col-4">
                        ...
                        <strong className="text-primary">
                          RS{s.totalPurchased.toLocaleString()}
                        </strong>
                      </div>
                      <div className="col-4">
                      ...
                      <div className="col-4">
                        ...
                        >
                          RS{s.totalRemaining.toLocaleString()}
                        </strong>
                      </div>
                    </div>
                    {/* Progress bar */}
                    <div className="mt-2">
                      <div className="progress" style={{ height: "6px" }}>
                        <div
                          ...
                          style={{
                            width: `${Math.min((s.totalPaid / s.totalPurchased) * 100, 100)}%`,
                          }}
                        ></div>
                      </div>
                      <small className="text-muted">
                        {((s.totalPaid / s.totalPurchased) * 100).toFixed(0)}%
                        paid
                      </small>
                    </div>
                  </div>
                ...
                <tbody>
                  {filtered.map((e, i) => (
                    ...
                    >
                      ...
                      <td>RS{e.purchasePrice.toLocaleString()}</td>
                      ...
                      >
                        RS{e.remainingAmount.toLocaleString()}
                      </td>
                      <td>
                        {e.remainingAmount <= 0 ? (
            ...
            <div className="modal-content border-0 shadow-lg">
              ...
              <div className="modal-body">
                <div className="alert alert-info py-2 mb-3">
                  ...
                  {selectedEntry.amountPaid.toLocaleString()} |{" "}
                  <strong className="text-danger">
                    Remaining: RS
                    {selectedEntry.remainingAmount.toLocaleString()}
                  </strong>
                </div>
                <label className="form-label fw-semibold">
                  Payment Amount (RS)
